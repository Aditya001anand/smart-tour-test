        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{
              ...
              fontSize: '1.3rem', boxShadow: '0 4px 15px rgba(9,132,227,0.3)',
            }}>🅰️</div>
            <div>
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1.1 }}>Universal Translator</h1>
              <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 500, letterSpacing: '0.5px' }}>MULTIMODAL AI AGENT</p>
            </div>
          </div>
          <Link href="/dashboard" className="btn-secondary" style={{ padding: '8px 16px', fontSize: '0.82rem', background: 'transparent' }}>Back to Dashboard</Link>
        </div>
      </header>
      ...
      <main style={{ maxWidth: '800px', margin: '40px auto 0', padding: '0 24px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <div className="glass-card animate-fade-in-up" style={{ padding: '40px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
            <button onClick={() => setMode('text')} className={mode === 'text' ? 'btn-primary' : 'btn-secondary'} style={{ flex: 1, background: mode === 'text' ? '#0984e3' : 'rgba(255,255,255,0.05)' }}>📝 Text</button>
            <button onClick={() => setMode('image')} className={mode === 'image' ? 'btn-primary' : 'btn-secondary'} style={{ flex: 1, background: mode === 'image' ? '#0984e3' : 'rgba(255,255,255,0.05)' }}>📸 Image</button>
            <button onClick={() => setMode('audio')} className={mode === 'audio' ? 'btn-primary' : 'btn-secondary'} style={{ flex: 1, background: mode === 'audio' ? '#0984e3' : 'rgba(255,255,255,0.05)' }}>🎙️ Audio</button>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <button type="submit" className="btn-primary" disabled={loading} style={{ padding: '14px', fontSize: '1.1rem', marginTop: '10px', background: '#0984e3' }}>
              {loading ? "Translating with Gemini..." : "Translate Directly"}
            </button>
        ...
        {translationResult && (
          <div className="glass-card animate-fade-in-up" style={{ padding: '40px', borderTop: '4px solid #0984e3' }}>
