...
function FoodContent() {
  ...
  const fetchFood = async (queryToFetch) => {
    if (!lat || !lon) {
      setError("Location coordinates (lat, lon) are missing. Please allow location access on the Home page/Dashboard.");
      setLoading(false);
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/food?lat=${lat}&lon=${lon}&q=${encodeURIComponent(queryToFetch)}`);
  ...
  useEffect(() => {
    // Only automatically fetch on mount if there's lat/lon
    // initialQuery can be empty
    if (lat && lon) {
      fetchFood(initialQuery);
    }
      ...
      <header style={{
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', fontWeight: 800, color: '#fff', lineHeight: 1.1 }}>Food & Cravings</h1>
              <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.8)', fontWeight: 500, letterSpacing: '0.5px' }}>LOCAL DINING DIRECTORY</p>
            </div>
          </div>
